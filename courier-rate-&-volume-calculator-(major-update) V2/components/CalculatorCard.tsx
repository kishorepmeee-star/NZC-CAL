...
const CalculatorCard: React.FC<CalculatorCardProps> = ({ title, icon, children, titleColorClass }) => {
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
      <div className="flex items-center mb-6">
        ...
        <h2 className={`text-3xl font-semibold ${titleColorClass || 'text-brandRed'} font-heading uppercase tracking-wide`}>{title}</h2>
      </div>
      <div>{children}</div>
    </div>
  );
